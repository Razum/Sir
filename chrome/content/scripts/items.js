...
SIR.rgba = {
	init: function() {
		...
		this.rgbaRect = document.getElementById("rgbaDiv");
	...
	showCode: function(R, G, B, opacity) {
...
SIR.txtShadow = {
	...
	showCode: function(horLen, verLen, blurRadius, color) {
...
SIR.boxShadow = {
	init: function() {
		...
		this.rect = document.getElementById("boxDiv");
	...
	showCode: function(horLen, verLen, blurRadius, color, inset) {
		var IEdirection = Math.abs(Math.round(Math.atan2(-verLen, horLen) * 180 / Math.PI) - 90) % 360;
		if (IEdirection < 0) {
			IEdirection = 180 + (180 + IEdirection);
		}
...
SIR.borderRadius = {
	init: function() {
		...
		this.rect = document.getElementById("borderRadiusDiv");
...
SIR.txtColumn = {
	...
	showCode: function(count, gap, rule) {
		var str = "";
		str += "/* Opera 11+*/\n";
	...
	emtopx: {
	   emtopxIsOk: null, 
        ...
        Validation: function(obj, type){
            ...
            switch(type){
                case "em": par = /^([0-9]\.[0-9]{3}|[1-9][0-9]\.[0-9]{3}|[1-9][0-9]|[1-9])$/.test(obj.value); this.emtopxError.setAttribute("hidden", par); break;
                case "px": par = /^([1-9][0-9]{2}|[1-9][0-9]|[1-9])$/.test(obj.value); this.emtopxError.setAttribute("hidden", par); break;
            }
        ...
        emtopxConvert: function() {
            ...
            switch(mes){
                case "em": this.result.value = (val * base).toFixed() + "px"; break;
                case "px": this.result.value = ((val / base).toFixed(2)).replace(/(\.?0{1,2})$/, "") + "em"; break;
            }
...
SIR.Cleaner = {
    ...
    onParamsChange: function() {
        this.delAttr.setAttribute("disabled", this.delTags.checked);
	...
	txtClean: function() {
       ...
       if(this.delAttr.checked && !this.delTags.checked){
...
SIR.outline = {
	...
	showCode: function(width, stl, color, offs) {
